<template>
	...
	>
		...
		<span slot="when_sent" slot-scope="when_sent">
	      {{ when_sent_display(when_sent) }}
	    </span>
	</a-table>
</template>
...
const columns = [{
	...
	dataIndex: 'when_sent',
	width: '10%',
	scopedSlots: { customRender: 'when_sent' }
}];
...
export default {
	...
	mounted() {
		...
		this.fetchEmailQueue()
			.then((response) => {
				this.loading = false;
				let results = response.data.map( obj => JSON.parse(obj));
